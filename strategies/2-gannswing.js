...
method.gannswing= function(candle, period) {
  var loma= function(period) {
    ...
    log.debug('--------------------------------------------------------------------------------------');
    log.debug('SMA');
    log.debug('inReal', this.history.low);
    log.debug('startIdx', 0);
    log.debug('endIdx', this.history.low.length - 1);
    log.debug('optInTimePeriod', period);
    log.debug('optInNbDev', 1);
    ...
    return talib.execute({
      ...
      log.debug('this.swing.lowma', data);
    })
  }.bind(this);
